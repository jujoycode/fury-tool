...
export class GitManage extends Command {
	...
	async prepare(): Promise<void> {
		// 0. Child Command 선택
		Object.assign(this.gitInfo, await this.Prompter.ask(GIT_INIT_PROMPT))
	}
	...
	private async pushGit() {
		...
		Object.assign(this.gitInfo, await this.Prompter.ask(COMMIT_INFO))
	...
	private async mergeGit() {
		...
		BRANCH_LIST[0].choices = sAllBranchList
			...
			.map(sBranch => {
				...
				return { name: sBranchName, value: sBranchName }
			})

		// 3. 대상 Branch 취득 (prompt)
		const branchInfoResponse = await this.Prompter.ask(BRANCH_LIST)
	...
	private async branchManage(): Promise<void> {
		...
		Object.assign(this.gitInfo, await this.Prompter.ask(BRANCH_COMMAND))
		...
		switch (this.gitInfo.branchCommand) {
			...
			case 'create': {
				...
				// 2-3. 필요 정보 취득 (prompt)
				Object.assign(this.gitInfo, await this.Prompter.ask(BRANCH_LIST))
				Object.assign(this.gitInfo, await this.Prompter.ask(BRANCH_INFO))
